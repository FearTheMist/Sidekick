...
- 统一控制台：在一个页面中管理 Providers、MCP、Permissions 和 General 设置
- 分级权限：终端命令按读取、项目执行、项目变更、工作区外访问、网络、高风险操作分类授权
...
1. 安装扩展。
2. 执行 `Sidekick: Open Control Center`。
...
6. 如需调整默认授权行为，配置 `sidekick.permissions` 或在控制台中修改 Permissions。
...
当前控制台中的模型 `Endpoint Type` 选项已收敛为以上 3 种。历史的 `OPENAI_COMPATIBLE*` 配置在保存时会自动归一化为 `OPENAI` 或 `OPENAI_RESPONSE`。
...
- `sidekick.permissions`：终端权限默认策略，支持 `allow`、`ask`、`deny`
...
当前仅支持基于 HTTP 的远程 MCP Server，不支持本地 `stdio` 方式。

## 常用命令

- `Sidekick: Open Chat`
- `Sidekick: Open Control Center`
...
- `npx vsce package` 会先自动执行 `npm run compile`
